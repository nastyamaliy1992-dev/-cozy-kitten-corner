...
function bindGame(){document.querySelectorAll('[data-room]').forEach(b=>b.addEventListener('click',()=>{if(actionLock)return;state.room=b.dataset.room;soundOn=true;persist();render();startRoomMusic(state.room)}));
document.querySelector('[data-action="pet"]')?.addEventListener('click',()=>{if(!state.sleeping){soundOn=true;startRoomMusic(state.room);purr();state=petKitten(state);persist();render()}});
