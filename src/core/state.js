...
export function createInitialState(name,lang='ru'){return {schemaVersion:1,createdAt:Date.now(),updatedAt:Date.now(),name,room:'living',sleeping:false,sleepStartedAt:null,lastSeenAt:Date.now(),needs:{hunger:78,thirst:82,cleanliness:88,mood:86,energy:80,toilet:84,health:100,happiness:88},economy:{coins:250,xp:0,level:1},inventory:{food:{starterFish:3,milk:2},clothes:[],furniture:[]},settings:{language:lang,reduceMotion:false}}}
...
export function tickState(s,seconds=10){const n=structuredClone(s),f=seconds/10;n.needs.hunger=clamp(n.needs.hunger-.18*f);n.needs.thirst=clamp(n.needs.thirst-.12*f);n.needs.toilet=clamp(n.needs.toilet-.08*f);n.needs.energy=clamp(n.needs.energy+(n.sleeping?.8:-.07)*f);n.lastSeenAt=Date.now();return recalc(n)}
export function petKitten(s){const n=structuredClone(s);n.needs.mood=clamp(n.needs.mood+4);return recalc(n)}
export function feedKitten(s,id='starterFish'){const n=structuredClone(s),count=n.inventory.food?.[id]||0;if(count<=0||n.needs.hunger>=96)return{state:n,ok:false};n.inventory.food[id]--;n.needs.hunger=clamp(n.needs.hunger+24);n.needs.mood=clamp(n.needs.mood+3);n.needs.toilet=clamp(n.needs.toilet-4);n.economy.xp+=6;return{state:recalc(n),ok:true}}
