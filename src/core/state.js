...
export function createInitialState(name,lang='ru'){return {schemaVersion:1,createdAt:Date.now(),updatedAt:Date.now(),name,room:'living',sleeping:false,sleepStartedAt:null,lastSeenAt:Date.now(),needs:{hunger:78,thirst:82,cleanliness:88,mood:86,energy:80,toilet:84,health:100,happiness:88},economy:{coins:250,xp:0,level:1},inventory:{food:{dryFood:4,wetFood:2,fishTreat:1},clothes:[],furniture:[],owned:[],equipped:null},bedroom:{lampOn:true},petting:{lastXpAt:0},settings:{language:lang,reduceMotion:false,musicVolume:.35,sfxVolume:.7,voiceVolume:.65,haptics:true,subtitles:true,graphics:'high',batterySaver:false,notifications:false,purchaseConfirm:true},movement:{x:50,y:78,target:null,facing:1}}}}
export function restoreState(){const s=loadSave();if(!s)return null;const now=Date.now(),m=Math.max(0,now-(s.lastSeenAt||s.updatedAt||now))/60000,n=structuredClone(s);n.movement??={x:50,y:78,target:null,facing:1};n.inventory??={food:{},clothes:[],furniture:[],owned:[],equipped:{}};n.inventory.food??={};n.inventory.clothes??=[];n.inventory.furniture??=[];n.inventory.owned??=[];n.settings={musicVolume:.35,sfxVolume:.7,voiceVolume:.65,haptics:true,subtitles:true,graphics:'high',batterySaver:false,notifications:false,purchaseConfirm:true,language:'ru',reduceMotion:false,...(n.settings||{})};n.lastSeenAt=now;n.needs.hunger=clamp(n.needs.hunger-Math.min(18,m*.08));n.needs.thirst=clamp(n.needs.thirst-Math.min(14,m*.06));n.needs.toilet=clamp(n.needs.toilet-Math.min(14,m*.05));if(n.sleeping){n.needs.energy=clamp(n.needs.energy+Math.min(48,m*.28));if(n.needs.energy>=96||m>=240){n.sleeping=false;n.sleepStartedAt=null}}else n.needs.energy=clamp(n.needs.energy-Math.min(12,m*.04));recalc(n);saveGame(n);return n}
