...
function ensure(){if(ctx)return;ctx=new (window.AudioContext||window.webkitAudioContext)();master=ctx.createGain();master.gain.value=.16;master.connect(ctx.destination)}
...
export function purr(duration=2.4){ensure();if(ctx.state==='suspended')ctx.resume();const end=ctx.currentTime+duration;const o=ctx.createOscillator(),g=ctx.createGain(),lfo=ctx.createOscillator(),lg=ctx.createGain();o.type='sawtooth';o.frequency.value=27;lfo.frequency.value=23;lg.gain.value=.10;lfo.connect(lg);lg.connect(g.gain);g.gain.value=.12;o.connect(g);g.connect(master);o.start();lfo.start();g.gain.setValueAtTime(.01,ctx.currentTime);g.gain.linearRampToValueAtTime(.22,ctx.currentTime+.2);g.gain.linearRampToValueAtTime(.01,end);o.stop(end);lfo.stop(end)}
